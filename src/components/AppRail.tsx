...
export function AppRail() {
  ...
  return (
    <aside
      className="app-rail flex w-9 shrink-0 flex-col items-stretch border-r border-terminal-border bg-terminal-chrome py-1"
      aria-label="App rail"
    >
      <div
        className="mx-auto mb-0.5 flex h-6 w-6 items-center justify-center rounded-[2px] bg-accent/15 text-[10px] font-bold leading-none text-accent"
        title="Flow Terminal"
      >
        Φ
      </div>

      <div className="mx-auto my-0.5 h-px w-4 bg-terminal-border" />
...
function RailButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className="rail-btn relative mb-px flex h-7 w-full items-center justify-center transition-colors"
    >
      {children}
      {badge != null && (
        <span className="absolute right-0.5 top-0.5 flex h-3 min-w-3 items-center justify-center rounded-full bg-accent/25 px-0.5 font-mono text-[7px] font-semibold leading-none text-accent">
          {badge > 9 ? '9+' : badge}
        </span>
      )}
    </button>
