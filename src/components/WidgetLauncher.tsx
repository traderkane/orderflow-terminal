...
export function WidgetLauncher() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/70 p-10 backdrop-blur-[2px]">
      <div className="w-full max-w-xl overflow-hidden rounded-[2px] border border-terminal-border bg-terminal-panel shadow-2xl">
        <div className="flex items-center justify-between border-b border-terminal-border px-3 py-2.5">
          ...
          <button
            ...
            className="rounded-[2px] border border-terminal-border px-2 py-0.5 font-mono text-[10px] uppercase tracking-wider text-zinc-500 hover:border-terminal-border-strong hover:text-zinc-200"
          >
            Esc
          </button>
        </div>
        <div className="grid grid-cols-2 gap-1.5 p-2.5">
          {LAUNCHABLE_WIDGET_TYPES.map((type) => (
            <button
              ...
              className="rounded-[2px] border border-terminal-border bg-terminal-elevated px-3 py-2.5 text-left transition hover:border-up/35 hover:bg-up/[0.04]"
            >
