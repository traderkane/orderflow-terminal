...
1. **Chart** — TradingView/MMT-style workspace: **Candles | Footprint** mode switch; left icon drawing rail (select / H-Line / Trend / Rect / Fib / Magnet / Eraser / Clear), bottom **layer dock** (favorites **Heatmap / VWAP / CVD** + **Layers** checklist for Profile / Bubbles / Bars / Count / Liqs), TF pills **1m / 5m / 15m / 1h**; Footprint mode paints clustered sell|buy volumes inside each bar with **Volume|Delta|Total** grading, delta tint + imbalance outlines + stacked/diagonal imbalance chains (≥3 bars, same or ±1 step) + naked POC diamonds until traded through (best at 1m; denser/compact on higher TFs); selected-drawing properties mini-panel (color / width / extend L-R / delete); magnet snap to candle OHLC+mid; maximize chart from header (**F** / Esc); drag-to-move drawings with handles; **session VWAP** (Day/Week/24h anchors, multi-plot) / **Bars** (volume|delta intensity) / **Count** (buy vs sell trade-count dual histogram); layer visibility persisted (`flow-terminal-chart-layers-v1`); drawings (+styles) persisted per symbol
2. **Order Book / DOM** — MMT-style price-centered ladder: heat-graded bid/ask sizes, session **Buys / Sells / Delta**, tick grouping (1/2/5/10), **USD|COIN**, **Center|Auto|Free** scroll, Reset clears session; prefs `flow-terminal-dom-prefs-v1`; chart↔DOM↔tape hover cohesion (shared `hoverPrice`)
3. **Trades Tape** — scrolling aggressor buys/sells with size highlighting; dense toolbar **USD|COIN** + **Min** size filter (units follow mode, prefs `flow-terminal-tape-prefs-v1`); row hover syncs chart line + DOM highlight; click briefly flashes that price on the chart
...
7. **Footprint** — detail side table of clustered bid/ask volume with **Volume|Delta|Total** visual grading (shared prefs `flow-terminal-footprint-grade-v1` with chart Footprint mode)
